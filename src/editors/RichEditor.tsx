...
export function RichEditor({
  ...
  useEffect(() => {
    ...
    const instance = markdownEditor(root.current!, tab.text)
      .use(history)
      .use(clipboard)
      .use(richPlugins(() => current.current.path, onError))
      .use(
        $prose(
          (ctx) =>
            new Plugin({
        ...
        editorActions.set(`${tab.id}:rich`, (action, value) => {
          ...
          instance.action((ctx) => {
            ...
            const commands = {
              ...
              rowAdd: addRowAfter,
              rowDelete: deleteRow,
              columnAdd: addColumnAfter,
              columnDelete: deleteColumn,
            };
            if (action in commands) {
              commands[action as keyof typeof commands](state, dispatch);
              return;
            }
            const marks = {
              bold: 'strong',
            ...
            if (action in marks) {
              const mark = schema.marks[marks[action as keyof typeof marks]];
              if (mark) toggleMark(mark)(state, dispatch);
            }
            if (action === 'heading')
              setBlockType(value === '0' ? schema.nodes.paragraph : schema.nodes.heading, {
                level: Number(value || 1),
              })(state, dispatch);
            if (action === 'bullet' || action === 'task') {
            ...
            if (action === 'table')
              instance.action(callCommand(insertTableCommand.key, { row: 3, col: 2 }));
            if (action === 'rule')
              dispatch(state.tr.replaceSelectionWith(schema.nodes.hr.create()));
            if (action === 'link' && value !== undefined) {
              if (!value)
                dispatch(
                  state.tr.removeMark(state.selection.from, state.selection.to, schema.marks.link),
                );
                ...
                dispatch(
                  state.tr
                    ...
                    .addMark(
                      ...
                      state.selection.from + value.length,
                      schema.marks.link.create({ href: value }),
                    ),
                );
              else
                dispatch(
            ...
            if (action === 'image' && value)
              dispatch(
                state.tr.replaceSelectionWith(schema.nodes.image.create({ src: value, alt: '' })),
              );
            if (action === 'selectAll')
  ...
  useEffect(() => {
    if (!ready || !visible || !editor.current || tab.text === lastPublished.current) return;
    editor.current.action((ctx) => {
      const view = ctx.get(editorViewCtx);
      const next = ctx.get(parserCtx)(tab.text);
      lastPublished.current = tab.text;
  ...
  useEffect(() => {
    if (ready)
      editor.current?.action((ctx) => {
        const view = ctx.get(editorViewCtx);
        view.dispatch(view.state.tr.setMeta('document-path', tab.path));
      });
  }, [tab.path, ready]);
  function find(next: boolean) {
    editor.current?.action((ctx) => {
      ...
      const selected =
        matches.find((m) => m.from >= (next ? view.state.selection.to : 0)) || matches[0];
      view.dispatch(
        view.state.tr
          .setSelection(TextSelection.create(view.state.doc, selected.from, selected.to))
          .scrollIntoView(),
      );
      setCount(`${matches.indexOf(selected) + 1} / ${matches.length}`);
    });
  }
  return (
