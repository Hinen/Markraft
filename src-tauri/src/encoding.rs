...
pub fn decode(bytes: &[u8]) -> Result<(String, Encoding), String> {
    let (data, encoding) = if bytes.starts_with(&[0xef, 0xbb, 0xbf]) {
        ...
        (&bytes[2..], Encoding::Utf16Le)
    } else if bytes.starts_with(&[0xfe, 0xff]) {
        (&bytes[2..], Encoding::Utf16Be)
    } else {
        (bytes, Encoding::Utf8)
    };
    let text = match encoding {
        Encoding::Utf8 | Encoding::Utf8Bom => String::from_utf8(data.to_vec())
            .map_err(|_| "지원하지 않는 인코딩입니다. UTF 파일만 열 수 있습니다.")?,
        _ => {
            if data.len() % 2 != 0 {
                return Err("잘못된 UTF-16 파일입니다.".into());
            }
    ...
    match encoding {
        ...
        Encoding::Utf16Le | Encoding::Utf16Be => {
            ...
            for word in text.encode_utf16() {
                bytes.extend(if encoding == Encoding::Utf16Le {
                    word.to_le_bytes()
                } else {
                    word.to_be_bytes()
                });
